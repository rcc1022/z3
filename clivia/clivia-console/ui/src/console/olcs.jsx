...
import { Col, Row, Collapse, List, Avatar, Badge, Popover, Input, Button, Space } from 'antd';
import { CloseCircleOutlined, SmileOutlined, PictureOutlined, SendOutlined, CodeOutlined } from '@ant-design/icons';
...
class Olcs extends React.Component {
    ...
    timer = () => {
        service('/olcs/member/query', { size: this.state.all.length }).then(data => {
            ...
            for (let a of data.all) {
                if ((a.uid && a.uid.indexOf(this.state.nick) > -1) || (a.nick && a.nick.indexOf(this.state.nick) > -1) || (a.memo && a.memo.indexOf(this.state.nick) > -1)) {
                    all.push(a);
                }
            }
    ...
    avatar = (user) => user.avatar ? <LazyLoadImage className='lazy-avatar' src={user.avatar} /> : <Avatar>{(user.nick || 'C').substring(0, 1)}</Avatar>;

    name = (user) => {
        let unread = this.state.unread[user.id];
        let name = user.memo ? <span>[{user.uid}]{user.nick}<span className="olcs-user-memo">({user.memo})</span></span> : <span>[{user.uid}]{user.nick}</span>;
        if (!unread || unread === 0)
            return name;

        return <Badge count={unread} offset={[10]}>{name}</Badge>;
    }
