...
export default {
    components: { Sign, Home, Record, Invite, Kefu, Deposit, Withdraw, Mine, Rule },
    data() {
        return {
            page: -2,
            splash: {
    ...
    mounted() {
        keyvalue('setting.splash.', data => {
            if (data['setting.splash.wei-xin'] === '1' && navigator.userAgent.toLowerCase().indexOf('micromessenger') === -1) {
        ...
        if (location.search && location.search.indexOf('invite=') > -1) {
            let code = location.search.substring(location.search.indexOf('invite=') + 7);
            if (code.indexOf('&') > -1)
    ...
    methods: {
        ...
        navigate: function (index) {
            this.navigator = 99;
            setTimeout(() => { this.navigator = index }, 10);
        },
...
<template>
    ...
    <div v-if="page === 0" id="sign-in">
        <div>
            ...
            <div class="sign-in-jump">页面自动<span class="jump" @click="jumpNow">跳转</span>等待时间：<span class="second">{{
                enter }}</span>秒</div>
    ...
    <div v-if="page === 2" id="body">
        ...
        <Deposit v-else-if="navigator === 4" @navigate="index => navigator = index" />
        <Withdraw v-else-if="navigator === 5" @navigate="index => navigator = index" />
        <Mine v-else-if="navigator === 6" @navigate="index => navigator = index" :illegal="illegal" />
