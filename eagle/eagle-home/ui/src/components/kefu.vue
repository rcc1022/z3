...
export default {
    ...
    mounted() {
        ...
        keyvalue('setting.home.ke-fu.', data => {
            ...
            if (this.kefu != '')
                this.kefu = url(this.kefu);
            this.qq = data['setting.home.ke-fu.qq'] || '';
            if (this.qq) {
                let qqs = this.qq.split(',');
                if (qqs.length > 0)
                    this.qqUrl = url(qqs[randomMax(qqs.length)]);
            }
    ...
    methods: {
        ...
        getUnread() {
            if (this.online || !this.unreadable)
                return;
        ...
        onemoji(e) {
            ...
            this.$refs.textarea.value = this.$refs.textarea.value.substring(0, this.$refs.textarea.selectionStart) + e.i + this.$refs.textarea.value.substring(this.$refs.textarea.selectionEnd);
        },
        picture() {
        ...
        send() {
            service('/olcs/ask', { genre: 'text', content: this.$refs.textarea.value }, data => {
                if (data === null)
                    return;
    ...
    <div v-if="online" class="olcs">
                ...
                <div v-if="message.replier" class="olcs-message-replier">
                    ...
                    <div class="olcs-message-content">
                        ...
                        <div v-else-if="message.genre === 'text'" class="olcs-message-text">
                            <div v-for="(line, index) in message.content" :key="index">{{ line }}</div>
                        </div>
                        <div v-else-if="message.genre === 'faq'" class="olcs-message-faq">
                            <div class="olcs-message-faq-title">常见问题</div>
                            <div :class="'olcs-message-faq-subject' + (faqs[faq.subject] ? ' olcs-message-faq-subject-ask' : '')"
                                v-for="(faq, index) in message.content" @click="askFaq(faq)">{{ index + 1 }}.{{
                                    faq.subject
                                }}</div>
                        </div>
        ...
        <div class="olcs-tools">
            ...
            <div class="olcs-tool-image" @click="picture">
                <svg viewBox="64 64 896 896" focusable="false" data-icon="picture" width="1em" height="1em"
                    fill="currentColor" aria-hidden="true">
                    <path
            ...
            <div class="olcs-tool-file"><input type="file" @change="upload" ref="uploader" accept="image/*" /></div>
